...
namespace lvalonexrumia
{
	...
	{
		// public static bool lowerthanhalf()
		// {
		...
		// 	}
		// }
		public static Int32 hpfrompercent(Unit unit, int percent, bool maxhp)
		{
			if (unit == null)
			{
				return 0;
			...
			if (maxhp)
			{
				return Convert.ToInt32(Math.Round((double)unit.MaxHp * percent / 100, MidpointRounding.AwayFromZero));
			}
			else
			{
				return Convert.ToInt32(Math.Round((double)unit.Hp * percent / 100, MidpointRounding.AwayFromZero));
			}
		}
