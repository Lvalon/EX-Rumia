	...
	public sealed class sedeepbleed : StatusEffect
	{
		public int truecount
		{
			get
			{
				if (Owner != null)
				{
					return (int)Math.Ceiling(Owner.MaxHp * 0.02f);
				}
				else
				{
					return 0;
		...
		{
			Count = (int)Math.Ceiling(unit.MaxHp * 0.02f);
		...
		private IEnumerable<BattleAction> OnDamageReceived(DamageEventArgs args)
		{
			...
			if (damageInfo.DamageType == DamageType.Attack)
			{
				...
				yield return new ChangeLifeAction(-(int)Math.Ceiling(args.Target.MaxHp * 0.02f), args.Target);
			}
			yield break;
		}
